...
public class King : MonoBehaviour, ISelectable {
    ...
    public void OnSelected() {
        ...
        CheckAndAdd(enemyObjects, _row + 1, _column ,opponentTag);
        CheckAndAdd(enemyObjects, _row + 1, _column + 1, opponentTag);
        CheckAndAdd(enemyObjects, _row + 1, _column - 1, opponentTag);
        CheckAndAdd(enemyObjects, _row, _column + 1, opponentTag);
        CheckAndAdd(enemyObjects, _row, _column - 1, opponentTag);
        CheckAndAdd(enemyObjects, _row - 1, _column, opponentTag);
        CheckAndAdd(enemyObjects, _row - 1, _column + 1, opponentTag);
        CheckAndAdd(enemyObjects, _row - 1, _column - 1, opponentTag);
    ...
    public void CheckAndAdd(List<GameObject> l1, int r, int c,string tag) {
        if ((r > 7 || r < 0) || (c > 7 || c < 0)) { return; }
        GameObject obj = PiecesHandler.instance.GetGameObjectOnTile(r, c);
        if (obj) { if (obj.tag == tag) { enemyObjects.Add(obj); } return; }
